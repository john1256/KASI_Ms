...
def Other_stuff_curved(z, parm): # parm[0] = Omegam, parm[1] = Omegalamb, d_p = c/H0*Other_stuff_flat
    ...
    grid_dist = np.array([quad_vec(E_inverse_curved, 0,n, args=(Omegam, Omega_L))[0] for n in grid])
    if np.abs(Omegak) < 1e-14: # flat universe
        grid_dist = grid_dist
    elif Omegak > 1e-14: # open universe
        grid_dist = 1/np.sqrt(Omegak)*np.sinh(np.sqrt(Omegak)*grid_dist)
    else: # closed universe
        grid_dist = 1/np.sqrt(-Omegak)*np.sin(np.sqrt(-Omegak)*grid_dist)
...
def D_M_flat(z, parm): # return D_M(z) = c/H0*Other_stuff_flat
    ...
    D_M = c/H0*np.array([quad_vec(E_inverse_curved, 0,zval, args=(Omegam,Omegalamb))[0] for zval in z])
    return D_M
def D_V_flat(z,parm):
    c = 299792.458 # speed of light in km/s
    Omegam, Omegalamb, H0 = parm
...
def Markov(func_SN, func_BAO, paramk,paramkp1,SNdata,BAOdata, prior, lnprior):
...
def MCMC(func_SN, func_BAO, paraminit,SNdata,BAOdata, nstep,normal_vec,prior): # param0 = [H0, Omegam, Omegalamb]
    """_summary_
    ...
    for k in tqdm(range(nstep)):
        ...
        param0, loglikelihood = Markov(func_SN, func_BAO, param0, paramkp1,SNdata,BAOdata, prior, lnprior) #loglikelihood = -chisq
