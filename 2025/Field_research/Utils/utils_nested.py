...
def prior_SN_curved(x):
    ...
    prior = x * np.array([20,6]) + np.array([0,-3]) 
    return prior
...
def Loglikelihood_SN_BAO_flat(parm):
    loglikelihood_SN = ufb.SN_Loglikelihood(ufb.Other_stuff_flat,parm, sndata)
...
def Loglikelihood_SN_BAO_curved(parm):
    loglikelihood_SN = ucb.SN_Loglikelihood(ucb.Other_stuff_curved,parm, sndata)
...
def prior_SN_BAO_curved(x):
    # Omegam in [0,20], Omegalamb in [-3,3], H0 in [4.66594018, 35.39070654]
    prior = x * np.array([20, 6, 35.39070654 - 4.66594018]) + np.array([0, -3, 4.66594018])
    return prior
